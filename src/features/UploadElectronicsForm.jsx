...
const UploadElectronicsForm = () => {
    ...
    const loadDropdownData = async () => {
      ...
      try {
        ...
        const processedDropdowns = {
          ...
          districts: [],
          cities: [],
          electronicsBrand: response.electronicsBrand?.data || response.electronicsBrand || [],
          electronicsModel: [],
        };

        console.log("✅ Processed dropdown data:", processedDropdowns);
        console.log("📱 Electronics brands count:", processedDropdowns.electronicsBrand.length);
  ...
  // Load electronics models when brand changes
  useEffect(() => {
    const loadElectronicsModels = async () => {
      ...
      try {
        console.log("Loading electronics models for brand:", formData.brand_id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("📱 Submitting electronics form data:", submissionData);

      // Use electronics form service instead of property form service
  ...
  useEffect(() => {
    const loadElectronicsDataForEdit = async () => {
      ...
      console.log("🔄 Loading electronics data for edit mode, ID:", id);
      
      try {
        dispatch(setLoading(true));
        dispatch(setApiError(null));
        
        // Fetch electronics data from API
        const response = await api.get(`/electronics/${id}/edit`);
        const result = response.data;
        
        console.log("📦 Electronics API response:", result);
        
        if (result && (result.data || result.status)) {
          const electronicsData = result.data || result;
          console.log("✅ Electronics data received:", electronicsData);
          ...
          if (electronicsData.state_id) {
            console.log("🌍 Loading districts for state:", electronicsData.state_id);
            await loadDistricts(electronicsData.state_id);
            
            // Load cities after districts are loaded
            if (electronicsData.district_id) {
              console.log("🏙️ Loading cities for district:", electronicsData.district_id);
              await loadCities(electronicsData.district_id);
            }
          }
          
          // Load electronics models if brand is selected
          if (electronicsData.brand_id && electronicsData.brand_id !== "other") {
            console.log("📱 Loading electronics models for brand:", electronicsData.brand_id);
            try {
          ...
          console.log("✅ Electronics form populated successfully");
        } else {
          console.error("❌ Invalid electronics API response:", result);
    ...
    <>
      <div className="bg-gray-50 p-6 rounded-3xl w-full max-w-6xl shadow-[0_0_10px_rgba(176,_176,_176,_0.25)] mx-auto border border-[#b9b9b9] bg-[#f6f6f6]">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* Row 2 - Electronics specific fields */}
          ...
          <div className="grid grid-cols-1 gap-4 mb-6">
            <div>
              ...
              <DynamicInputs
                ...
                name="specifications"
                id="specifications"
                className="w-full h-[90px] px-4 py-3 rounded-[10px] border border-[#bfbfbf] 
                bg-white focus:outline-none"
