...
const CarKilometersFilter = ({
  ...
  const [selectedKilometersRanges, setSelectedKilometersRanges] = useState([]); // Array for multi-select
  ...
  const fetchKilometersRanges = async () => {
    setLoading(true);
    try {
      const response = await api.get("/car/get/kilometers");
  ...
  return (
    <div className="mb-4">
      ...
      >
        <h2 className="font-medium text-gray-800 flex items-center">
          <SpeedIcon className="mr-2 text-blue-500" style={{ fontSize: 18 }} />
          Kilometers
        ...
        {kilometersRanges.length > 0 ? (
          <div className="grid grid-cols-1 gap-2 mb-2">
            {kilometersRanges.map((range, index) => {
              const isSelected = selectedKilometersRanges.includes(range.value);
...
export default CarKilometersFilter;
