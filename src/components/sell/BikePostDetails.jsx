...
const BikeCard = ({
  ...
  const handleCardClick = (event) => {
    ...
    // Check if admin has not approved the bike
    if (adminapproved === 0 || adminapproved === false) {
      toast.warning("Waiting for admin approval. Your bike is under review.");
      return;
    }
    ...
    if (isNavigationDisabled()) {
      if (currentStatus === "sold" || currentStatus === "available") {
        toast.info("This bike has been sold and is no longer available");
      } else if (!currentPublishStatus) {
        toast.info("This bike is currently unpublished");
      }
      return;
    }
          ...
          {loading
            ...
            : bikes.map((bike) => (
              <BikeCard
                ...
                location={`${bike.district || ""}${bike.state ? `, ${bike.state}` : ""
                  }`}
                ...
                adminapproved={bike.is_approved || 0}
