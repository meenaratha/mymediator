...
const CarCard = ({
  ...
  const updateStatus = async (newStatus) => {
    ...
    try {
      const response = await api.patch(`/car/status/${id}`, {
        status: newStatus,
      });
  ...
  const updatePublishStatus = async (publishStatus) => {
    ...
    try {
      ...
      const response = await api.patch(`/car/publish/${id}`, {
  ...
  const handleCardClick = (event) => {
    ...
    // Check if admin has not approved the car
    if (adminapproved === 0 || adminapproved === false) {
      toast.warning("Waiting for admin approval. Your car is under review.");
      return;
    }
    ...
    if (isNavigationDisabled()) {
      if (currentStatus === "sold" || currentStatus === "available") {
        toast.info("This car has been sold and is no longer available");
      } else if (!currentPublishStatus) {
        toast.info("This car is currently unpublished");
      }
      return;
    }
          ...
          {loading
            ...
            : cars.map((car) => (
              <CarCard
                ...
                carTitle={car.title}
                location={`${car.district || ""}${car.state ? `, ${car.state}` : ""
                  }`}
                ...
                adminapproved={car.is_approved || 0}
