...
// Bike form submission service
class BikeFormService {
  ...
  mapValidationErrors(backendErrors) {
    ...
    Object.entries(backendErrors).forEach(([backendField, errorMessage]) => {
      ...
      if (Array.isArray(errorMessage)) {
        mappedErrors[frontendField] = errorMessage[0];
      } else {
  ...
  // Transform bike form data
  transformBikeFormData(formData, subcategoryId, urlId) {
    const payload = {
      // Required fields
      form_type: "bike",
      ...
      // Bike specification fields (always include these)
      engine_cc: formData.engine_cc || "",
      kilometers: parseInt(formData.kilometers) || formData.kilometers || "",
      fuel_type_id: parseInt(formData.fuel_type_id) || formData.fuel_type_id || "",
      number_of_owner_id: parseInt(formData.number_of_owner_id) || formData.number_of_owner_id || "",
    ...
    console.log("Transformed bike form data:", payload);
    return payload;
  }

  // Submit bike form with files
  async submitBikeForm(submissionData, slug, isEditMode = false) {
    try {
      const { urlId, subcategoryId, ...formData } = submissionData;

      // Transform form data for bike submission
      const payload = this.transformBikeFormData(formData, subcategoryId, urlId);
      ...
      // Debug the payload
      console.log("Final Bike FormData payload:");
      ...
      if (response.data && response.data.success === false) {
        return {
          success: false,
          error: response.data.message || "Bike submission failed",
      ...
      if (error.response) {
        ...
        switch (status) {
          ...
          case 422:
            return {
              success: false,
              error: "Bike form validation failed",
          ...
          default:
            return {
              success: false,
              error: data.message || "Bike submission failed",
  ...
  // Get bike subcategory ID
  getBikeSubcategoryId(slug) {
    // Adjust these IDs based on your backend subcategory setup
    const bikeSubcategoryMapping = {
      "bike": 17, // Based on your JSON example
      "motorcycle": 18,
      "scooter": 19,
      // Add more as needed
    };
    
    return bikeSubcategoryMapping[slug] || 17; // Default to 17
  }

  // Main submit method that determines subcategory automatically
  async submitForm(submissionData, slug, isEditMode = false) {
    // Get subcategory ID for bikes
    const subcategoryId = submissionData.subcategory_id || this.getBikeSubcategoryId(slug);
    
    return this.submitBikeForm(
      {
        ...submissionData,
        subcategoryId,
      },
...
export const bikeFormService = new BikeFormService();

// Export the main submit method for convenience
export const submitBikeForm = (submissionData, slug, isEditMode = false) =>
  bikeFormService.submitForm(submissionData, slug, isEditMode);

// Export for direct use
export default bikeFormService;